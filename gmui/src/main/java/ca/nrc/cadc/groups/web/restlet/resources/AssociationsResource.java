...
public class AssociationsResource extends AbstractResource implements Runnable {
    ...
    public void refresh() {
        ...
        try {
            Subject.doAs(getAuthorizedUser(), (PrivilegedExceptionAction<Void>) () -> {
                doRefresh();
                return null;
            });
        } catch (Exception exception) {
            handleRefreshException(exception);
        }
        LOGGER.info("Refreshing GMUI cache: OK");
    }

    private void handleRefreshException(final Exception exception) {
        if (exception instanceof PrivilegedActionException) {
            // This is a wrapper for the original exception.
            final Exception cause = ((PrivilegedActionException) exception).getException();
            handleRefreshException(cause);
        } else if (exception instanceof ResourceException) {
            throw (ResourceException) exception;
        } else {
            LOGGER.error("Error refreshing GMUI cache", exception);
     ...
     */
    @Post
    public void accept(final Representation ignored) {
        refresh();
    }
