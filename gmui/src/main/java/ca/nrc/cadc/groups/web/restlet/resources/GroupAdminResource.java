...
public class GroupAdminResource extends AbstractAssociateResource {
    ...
    public void remove(final Representation payload) throws Exception {
        ...
        if (associate.isUser()) {
            group.getUserAdmins().removeIf(user -> user.getHttpPrincipal().getName().equals(associate.getID()));
        } else {
            group.getGroupAdmins().removeIf(adminGroup -> adminGroup.getID().getName().equals(associate.getID()));
        }
